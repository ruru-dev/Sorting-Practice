...
const sumEvens = evenNums.reduce( (total, num) => {
  return total += num;
});
...
const weatherStates = weather.map(  (object) => {
 return object.weather_state_name;
})

//Alternative way to write above callback function.
// const weatherStates2 = weather.map( object => object.weather_state_name);
...
const idealTemp = weather.filter( (object) => {
  if (object.min_temp === 15.915) {
    return true;
  }
})

//Shorter way to write above callback function.
// const idealTemp2 = weather.filter( object => object.min_temp === 15.915);

console.log(idealTemp[0].id)